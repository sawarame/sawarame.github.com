...
import { TextField, Button, Stack, Snackbar, Alert, Tooltip } from '@mui/material';
...
export default function QR(): React.JSX.Element {
  ...
  const downloadQRCode = () => {
    ...
    a.href = pngUrl; a.download = 'qrcode.png';
          ...
          <div className={styles.container}>
            <div className={styles.layout}>
              ...
              <div className={common.card}>
                ...
                <Stack spacing={2.5}>
                  <TextField fullWidth label="URL または テキスト" variant="outlined" value={text}
                    onChange={(e) => setText(e.target.value)} placeholder="ここにURLやテキストを入力してください" multiline minRows={6} />
                  <Stack direction="row" spacing={1.5} flexWrap="wrap">
                    <Button variant="contained" startIcon={<DownloadIcon />} onClick={downloadQRCode} disabled={!text} className={styles.actionBtn}>
                      画像を保存
                    </Button>
                    <Tooltip title="クリップボードに画像としてコピー">
                      <span>
                        <Button variant="outlined" startIcon={<ContentCopyIcon />} onClick={copyToClipboard} disabled={!text} className={styles.actionBtn}>
                          画像をコピー
                        </Button>
                      </span>
                    </Tooltip>
                ...
                <div className={styles.qrWrap}>
                  {text ? (
                    <div className={styles.qrInner}>
                      <QRCodeCanvas value={text} size={240} level="H" includeMargin ref={qrRef} />
                    </div>
                  ) : (
