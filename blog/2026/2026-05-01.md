...
1.  **演算性能の測定（スコア化）**: 端末のCPUパワーを測定し、シングルコア・マルチコアそれぞれのスコアとランク（S〜F）を出します。
2.  **ブラウザ機能サポート状況**: 最新のWeb API（WebGPU, Passkeys, WebP等）がそのブラウザで使えるかをチェックします。
...
以下のようなシーンで使ってもらえると嬉しいです！

- **開発プロジェクトの要件定義**: 「Web快適度測定のシングルスコア1000以上を推奨環境とする」といった具体的な指標として。
- **格安スマホの性能チェック**: 買ったばかりのスマホが、今のWeb標準に対してどの程度の立ち位置なのかを確認する。
- **ブラウザによる性能差の確認**: 同じ端末でも、SafariとChromeでスコアがどう変わるか（JSエンジンの性能差）を比べる。

ぜひ、皆さんの手元のデバイスでも [Web快適度測定](/benchmark) を試してみてください！
スコア結果はSNS等でシェアしてもらえると、統計が取れて楽しいかもしれません！
